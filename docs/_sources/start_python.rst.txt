...
Python はインタプリタ言語というタイプのプログラミング言語で、テキスト形式のソースファイルを Python 実行用の本体プログラムに読込ませることで処理を実行できます。
...
* 変数 data に代入した bool 型のデータ True を **文字列の "1"** に変換する。
